...
const paletteTextureRef = ref<THREE.DataTexture | null>(null);

// 获取CSS变量值的函数
const getCSSVariable = (varName: string): string => {
  return getComputedStyle(document.documentElement).getPropertyValue(varName).trim() || '#1890ff';
};

// 基于主色调生成颜色数组
const generateColorsFromPrimary = (primaryColor: string): string[] => {
...
const currentColors = computed(() => {
  // 如果props.colors有值且不为默认值，使用props.colors
  const defaultColors = ['#5227FF', '#FF9FFC', '#B19EEF'];
  if (props.colors && props.colors.length > 0 &&
      JSON.stringify(props.colors) !== JSON.stringify(defaultColors)) {
    return props.colors;
  }
  // 否则从CSS变量获取主色调并生成
...
const initWebGL = () => {
  ...
  const paletteTex = makePaletteTexture(currentColors.value);
  paletteTextureRef.value = paletteTex;
  ...
  const io = new IntersectionObserver(
      entries => {
        ...
        const wasVisible = isVisibleRef.value;
        ...
        if (isVisible && !document.hidden) {
          if (!wasVisible) {
            ...
            // 重新启动定时检查
            if (!checkIntervalRef.value) {
              checkIntervalRef.value = window.setInterval(() => {
                if (!isVisibleRef.value || document.hidden) return;
                const newPrimaryColor = getCSSVariable('--primary-color');
...
const updatePaletteTexture = () => {
  if (!paletteTextureRef.value || !webglRef.value) return;
  ...
  paletteTextureRef.value.image.data = data;
  paletteTextureRef.value.needsUpdate = true;

  // 更新output中的uniform
  const output = webglRef.value.output;
  if (output && (output as any).output && (output as any).output.material) {
    const material = (output as any).output.material as THREE.RawShaderMaterial;
    if (material.uniforms && material.uniforms.palette) {
      material.uniforms.palette.value = paletteTextureRef.value;
    }
  }
};

// 监听CSS变量变化
const cssVariableObserverRef = ref<MutationObserver | null>(null);
const checkIntervalRef = ref<number | null>(null);
let lastPrimaryColor = getCSSVariable('--primary-color');

const watchCSSVariable = () => {
  ...
  cssVariableObserverRef.value = observer;

  // 定时检查作为备用方案，但只在组件可见时启动（在IntersectionObserver中处理）
};
...
onUnmounted(() => {
  ...
  if (cssVariableObserverRef.value) {
    try {
      cssVariableObserverRef.value.disconnect();
    } catch {
      /* noop */
    }
  }
  ...
  if (paletteTextureRef.value) {
    paletteTextureRef.value.dispose();
  }
  if (webglRef.value) {
