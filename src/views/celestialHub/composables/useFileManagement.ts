...
export function useFileManagement(
    ...
    onFilesUploaded?: (fileReferences: FileReference[]) => void
) {
    const {t} = useI18n()
    ...
    const uploadSelectedFiles = async (selectedFiles: File[]) => {
        ...
        await uploadFiles(selectedFiles, uploadOptions)
            .then((response) => {
                ...
                // 将上传成功的文件转换为FileReference并添加到文件引用列表
                if (response.data && response.data.length > 0 && onFilesUploaded) {
                    const fileReferences: FileReference[] = response.data.map((file: FileDocumentVO) => ({
                        id: file.id || null,
                        ...
                        bucketCode: null,
                        sysUserId: file.sysUserId || null,
                        courseId: file.courseId || null,
                        sessionId: file.sessionId || null,
                        isVectorized: file.isVectorized || null,
                        vectorCount: file.vectorCount || null
                    }))
                    onFilesUploaded(fileReferences)
                }
            })
            .catch(() => {
