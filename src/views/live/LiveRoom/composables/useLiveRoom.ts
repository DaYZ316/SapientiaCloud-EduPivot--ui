...
import type {HandRaiseState, LiveRoomChatMessage, LiveRoomVO, RemoteParticipantMedia} from '@/types/live'
...
import {type RemoteParticipant, type RemoteTrackPublication, type Room, RoomEvent, Track} from 'livekit-client'
...
export const useLiveRoom = (roomIdProp?: string | null, tokenProp?: string | null): LiveRoomResult => {
    ...
    const handleHandRaiseMessage = (payload: {
        ...
        if (existingIndex < 0) {
            ...
            // 监听通知关闭时，从列表中移除
            // 注意：notification 不提供关闭回调，这里简化处理
            // 实际产品中可以考虑使用 notification 的返回值来管理
        }
    }
